...
window.Planifolia.routerFetchPage = () => {
  ...
  if (
    ...
    path == "" && ((path = "index"), (window.location.href = "#/")),
    path.includes("?"))
  ) {
      ...
      .then((text) => {
        (window.PlanifoliaSettings.container.innerHTML = text),
          window.Planifolia.fetchComponents(
            window.PlanifoliaSettings.container
          ),
...
window.Planifolia.fetchComponents = (target) => {
  ...
  exts.forEach(async (e) => {
    let name = e.getAttribute("name"),
      componentUrl = window.Planifolia.resolveFile(name),
      text = "";
    componentCache[componentUrl] == undefined
      ? ((text = await fetch(componentUrl).then((res) => {
    ...
    for (const attr of e.attributes) {
      ...
      if (attrName.startsWith("@")) attrName = attrName.substr(1);
      else continue;
...
window.Planifolia.fetchElements = (target) => {
  target.querySelectorAll("script").forEach((s) => {
    ...
    window.PlanifoliaSettings.autoRewriteLinks &&
...
window.Planifolia.joinPaths = (...args) => {
  var output = "";
  for (const path of args) output += path.replace(/^\/|\/$/g, "") + "/";
...
window.Planifolia.resolveFile = (partialName) => {
  var basePath = window.PlanifoliaSettings.basePath,
    ...
    result = window.Planifolia.joinPaths(
      prefix,
      basePath,
      partialName + ".html"
    );
